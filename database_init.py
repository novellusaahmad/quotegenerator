...
def init_database():
    ...
    try:
        ...
        with app.app_context():
            ...
            # Ensure LoanNote table exists but do not seed default data
            if 'loan_notes' in db.inspect(db.engine).get_table_names():
                if LoanNote.query.count() == 0:
                    logger.info("Loan Notes table is empty - no default data seeded. Use install.sh to restore from backup if available.")
