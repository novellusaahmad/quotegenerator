...
# from quote_generator import QuoteGenerator  # Removed - functionality moved to PDF/Excel generators
# Export generators removed - functionality eliminated

# Excel generator removed - export functionality eliminated
...
def api_calculate():
    ...
    try:
        ...
        if rate_input_type == 'annual':
            annual_rate = safe_float(data.get('annual_rate'), 0)
        ...
        app.logger.info(f'Storing in session - Loan Type: {result.get("loan_type", "NOT_FOUND")}')
        app.logger.info(f'Storing in session - grossAmount: {result.get("grossAmount", "NOT_FOUND")}')
        app.logger.info(f'Storing in session - totalInterest: {result.get("totalInterest", "NOT_FOUND")}')
        app.logger.info(f'Storing in session - arrangementFee: {result.get("arrangementFee", "NOT_FOUND")}')
        app.logger.info(f'Storing in session - netAdvance: {result.get("netAdvance", "NOT_FOUND")}')
        app.logger.info(f'Storing in session - totalNetAdvance: {result.get("totalNetAdvance", "NOT_FOUND")}')
        
        # Store result in session for PDF/Excel generation with loan type identification
...
def generate_quote_document(id):
    ...
    try:
        ...
        result = quote_gen.generate_quote(
            loan_data=loan_data,
        ...
        # Save HTML content to temporary file and return
        with tempfile.NamedTemporaryFile(mode='w', suffix='.html', delete=False) as f:
            f.write(result['html_content'])
            temp_path = f.name
        
        return send_file(temp_path, as_attachment=True, download_name=f'quote_{quote.id}.html')
...
def get_loan_details(loan_id):
    ...
    try:
        ...
        loan_data = {
            ...
            # Date and term information
            'startDate': loan.start_date.strftime('%d/%m/%Y') if loan.start_date else '-',
            'endDate': loan.end_date.strftime('%d/%m/%Y') if loan.end_date else '-',
            'loanTerm': loan.loan_term if loan.loan_term else 0,
            'loanTermDays': loan.loan_term_days if loan.loan_term_days else 0,
            
            # User input parameters
            'amountInputType': loan.amount_input_type,
            'interestRate': float(loan.interest_rate) if loan.interest_rate else 0,
            'repaymentOption': loan.repayment_option,
            'paymentTiming': loan.payment_timing,
            ...
            'arrangementFeePercentage': float(loan.arrangement_fee_percentage) if loan.arrangement_fee_percentage else 0,
            ...
            'tranchesData': loan.tranches_data,
            
            # Calculation results for summary display
            ...
            'legalCosts': float(loan.legal_costs) if loan.legal_costs else 0,
            'siteVisitFee': float(loan.site_visit_fee) if loan.site_visit_fee else 0,
            ...
            'interestOnlyTotal': float(loan.interest_only_total) if loan.interest_only_total else 0,
            'interestSavings': float(loan.interest_savings) if loan.interest_savings else 0,
            'savingsPercentage': float(loan.savings_percentage) if loan.savings_percentage else 0,
